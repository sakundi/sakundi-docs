...
Several governments, including the Costa Rican one, offer an advanced digital identity system (Firma Digital) that allows residents or entities to sign documents with a digital identity validated by the Central Bank as a Certification Authority (CA) and authenticate themselves on various public or private websites and services. Zikuani, a project by Sakundi, is a zero-knowledge proof (ZKP) protocol that enables people to prove their identity in a privacy-preserving manner, today from either a Firma Digital credential (initially from Costa Rica) or a biometric passport (NFC chip, ICAO 9303) — with support for the mobile driver's license standard (mDL, ISO/IEC 18013-5) planned next. It provides a set of tools to generate and verify proofs, authenticate users, and perform proof verifications both on-chain and off-chain.
